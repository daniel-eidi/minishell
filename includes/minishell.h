...
/*   Updated: 2022/10/18 15:52:04 by mgaldino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			update_hashtable(char *key, void *new_value, \
								t_list	**hash_table);
...
void			open_fds(char **redir, t_pids_pipes *aux, \
							int n_cmd, int *have_file);
void			open_fdsfrk(char **redir, t_pids_pipes *aux, \
							int n_cmd, int *have_file);
void			free_data_after_exec_cmd(char **environ);
void			set_outfile_flags(int *fd, char *redir, \
							int *flags, int *have_file);
void			set_infile_flags(int *fd, char *redir, \
							int *flags, int *have_file);

// ---utils fork_exec

void			free_cmd_and_close_pipes(char **cmd, int n_cmd, \
										t_pids_pipes *aux);
